...
finalConv.Weights = randn(1,1,decoderNumChannels,numClass);
finalConv.Bias = randn(1,1,numClass)*0.00001 + 1;

smLayer = softmaxLayer('Name','Softmax-Layer');

pixelClassLayer = pixelClassificationLayer('Name','classification'); %, 'ClassNames', classesTbl.Name, 'ClassWeights', classWeights);
...
% lgraph = connectLayers(lgraph, 'Encoder-Section-5-DropOut','Decoder-Section-1-DepthConcatenation/in2');



% insert DACblock and RMPblock , the kernel in CEnet
lgraph = disconnectLayers(lgraph, 'Mid-DropOut', 'Decoder-Section-1-UpConv');

lgraph = DACblock(lgraph, 'Mid-DropOut', 1024);  % the conv2d size must be 1024, if not the laye will be wrong
lgraph = RMPblock(lgraph, 'DAC_add');
lgraph = connectLayers(lgraph, 'RMP_cat','Decoder-Section-1-UpConv');
